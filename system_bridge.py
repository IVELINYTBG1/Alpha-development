...
=============================================
Gives Nova and Simona access to the Linux environment they live in.
...
PHILOSOPHY:
  ...
  Nova's PFC decides WHEN to send a notification.
  Simona's insula decides WHAT to say in it.
...
_LOG = logging.getLogger("nova_simona.system")
...
class DBusBridge:
    ...
    def notify(self, summary: str, body: str = "",
        ...
        Nova uses this when her PFC decides something is worth surfacing.
        Simona uses this when her insula overflows and she needs to blurt.
        """
        try:
            if self._mode == "dasbus":
                ...
                notif.Notify(
                    "Nova & Simona", 0, icon, summary, body,
                ...
                iface.Notify("Nova & Simona", 0, icon, summary, body,
    ...
    def nova_notify(self, message: str, urgency: int = 1) -> bool:
        """Nova sends a notification in her own name."""
        return self.notify(
            summary="Nova",
            body=message,
            urgency=urgency,
    ...
    def simona_notify(self, message: str, urgency: int = 1) -> bool:
        """Simona sends a notification — usually more urgent."""
        return self.notify(
            summary="Simona!!",
            body=message,
...
class PipeWireBridge:
    ...
    def _init(self):
        try:
            import pulsectl
            self._pulse = pulsectl.Pulse("nova-simona")
            self._mode  = "pulsectl"
...
class SystemAction:
    ...
    Generated by NovaBrain or SimonaBrain when their PFC/Broca fires
    ...
    actor:   str            # "nova" | "simona"
    payload: dict = field(default_factory=dict)


class SystemBridge:
    """
    Top-level system bridge — Nova and Simona's interface to Linux.
    ...
    def execute(self, action: SystemAction) -> dict:
        ...
        try:
            if action.action == "notify":
                ...
                if action.actor == "nova":
                    ok = self.dbus.nova_notify(text, urgency)
                else:
                    ok = self.dbus.simona_notify(text, urgency)
    ...
    def describe_environment(self) -> str:
        ...
        Used as context for Nova's responses about the world.
        """
        snap = self.snapshot()
